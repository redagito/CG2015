...
bool load(const std::string& file, IResourceManager& manager, CGameSystem& system)
{
	...
	// Load game states which contain high level game logic and application control
...
bool loadGameState(const std::string& file, const std::string& name, IResourceManager& manager, CGameSystem& system)
{
	// Load json file
	Json::Value root;
	if (!load(file, root))
